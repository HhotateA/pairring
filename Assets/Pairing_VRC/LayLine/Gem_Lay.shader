...
Shader "HOTATE/Pairing/Gem_Lay"
{
	Properties
	{
		...
		_laylong ("レイの長さ",Range(0,1)) = 0.5
		_layalpha ("レイの透明度調節",float) = 1
		[HideInInspector]_laydist ("レイの見える距離",float) = 1.5
	}
	SubShader
	{
		...
		Pass
		{
            ...
            void geom(triangle v2g input[3], uint primID : SV_PrimitiveID, inout TriangleStream<g2f> output)
            {
				...
				if(primID < 0.1){
					o.vertex = mul(UNITY_MATRIX_P,pos1+float4( _layw/2,0.0,0.0,0.0)); o.laycast = 0; output.Append(o);
					o.vertex = mul(UNITY_MATRIX_P,pos1+float4(-_layw/2,0.0,0.0,0.0)); o.laycast = 0; output.Append(o);
					o.vertex = mul(UNITY_MATRIX_P,pos2+float4( _layw/2,0.0,0.0,0.0)); o.laycast = 1; output.Append(o);
					o.vertex = mul(UNITY_MATRIX_P,pos2+float4(-_layw/2,0.0,0.0,0.0)); o.laycast = 1; output.Append(o);
				}else if(primID < 1.1){
					o.vertex = mul(UNITY_MATRIX_P,pos1+float4(0.0, _layw/2,0.0,0.0)); o.laycast = 0; output.Append(o);
					o.vertex = mul(UNITY_MATRIX_P,pos1+float4(0.0,-_layw/2,0.0,0.0)); o.laycast = 0; output.Append(o);
					o.vertex = mul(UNITY_MATRIX_P,pos2+float4(0.0, _layw/2,0.0,0.0)); o.laycast = 1; output.Append(o);
					o.vertex = mul(UNITY_MATRIX_P,pos2+float4(0.0,-_layw/2,0.0,0.0)); o.laycast = 1; output.Append(o);
				}
				output.RestartStrip();
            }
