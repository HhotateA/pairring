//made by hhotatea
//2018/12/08
Shader "HOTATE/Pairing/Gem_himo"
{
	Properties
	{
		...
		_laycolor ("レイの色",Color) = (0.7,0.1,0.1,0.7)
		_layw ("レイの太さ",float) = 0.002
		//_laylong ("レイの長さ",Range(0,1)) = 0.0
		//_layalpha ("レイの透明度調節",float) = 0
		[HideInInspector]_laydist ("レイの見える距離",float) = 3.0
		_laysag ("たるみ",float) = 0.1
	}
	SubShader
	{
		Tags {"RenderType"="Opaque" "Queue" = "Overlay-602"}
		LOD 100
		Stencil {
            Ref 11
            Comp NotEqual
		...
		Pass
		{
			...
			uniform float _pow;
			uniform float _dist;
			uniform float4 _laycolor;
			uniform float _layw;
			uniform float _layalpha;
			uniform float _laylong;
            ...
            void geom(triangle v2g input[3], uint primID : SV_PrimitiveID, inout TriangleStream<g2f> output)
            {
				...
				float4 buf = tex2Dlod(_GrabTexture,float4(memouvunpack_lay(_pair),0.0,0.0));
				float4 pos1 = buf;
				float4 pos2 = mul(UNITY_MATRIX_MV,float4(0.0,0.0,0.0,1.0));
				float4 pos[21];
				for(int index=0;index<21;index++){ pos[index] = lerp(pos1,pos2,index/20.0) + curve(index/20.0);}
				float4 crossvec;
				o.laycast = 0;
				if(length(buf.xyz)>0.1){
					for(int index=0;index<20;index++){
						crossvec = float4(normalize(cross(float3(0.0,0.0,1.0),pos[index].xyz-pos[index+1].xyz)),0.0); //視線とレイに直行するベクトル(ビルボード方向)

						o.vertex = mul(UNITY_MATRIX_P,pos[index]+crossvec*_layw); output.Append(o);
						o.vertex = mul(UNITY_MATRIX_P,pos[index]-crossvec*_layw); output.Append(o);

						o.laycast += 1.0/19.0;

						o.vertex = mul(UNITY_MATRIX_P,pos[index+1]+crossvec*_layw); output.Append(o);
						o.vertex = mul(UNITY_MATRIX_P,pos[index+1]-crossvec*_layw); output.Append(o);
						output.RestartStrip();
					}
				}
			...
			fixed4 frag (g2f i) : SV_Target
			{
				float4 buf = tex2Dlod(_GrabTexture,float4(memouvunpack_lay(_pair),0.0,0.0));
				clip(length(buf)-0.2);
				clip(_laydist-distance(_WorldSpaceCameraPos,mul(UNITY_MATRIX_M,float4(0.0,0.0,0.0,1.0))));
				return fixed4(_laycolor.rgb,saturate(pow(saturate(i.laycast),_layalpha)-_laylong));
			}
			ENDCG
		}
